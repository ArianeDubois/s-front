...
const scrollCta = ref(null);
const scrollToSecondImage = () => {
    if (album.value) {
        $gsap.to(window, { scrollTo: album.value.children[1], duration: 0.5, ease: 'power2.inOut' });
    }
};
onMounted(() => {
    if (scrollCta.value) {
        $gsap.to(scrollCta.value, {
            opacity: 0,
            duration: 0.2,
            scrollTrigger: {
                trigger: album.value.children[1],
                start: "top center",
...
<template>
    <article>
        <div class="grid">
            <ul class="album-gallery" ref="album">
                <li v-for="(image, index) in page?.gallery ?? []" :key="index"
                    :class="index === 0 ? 'cover' : 'images'">
        ...
        <div ref="infos" class="infos">
            ...
            <NuxtLink prefetch :style="`color: ${page.color}; opacity:${children[pageIndex + 1] ? 1 : 0} `"
                :to="'/' + (children[pageIndex + 1] ? children[pageIndex + 1].id : '0')" class="button-nav">Next
            </NuxtLink>
        </div>
        <LazyAppNavProject />
    </article>
